...
├── allure_results/              # результаты тестов - отчет Allure
...
2. **Оформление заказа** — `tests/test_order.py`
   ...
   - клик по кнопке «Заказать» сверху и снизу страницы;
   - заполнение формы заказа — два набора данных;
   - проверка всплывающего окна подтверждения заказа.

   Итого 2 теста (2 точки входа и 2 набора данных)
